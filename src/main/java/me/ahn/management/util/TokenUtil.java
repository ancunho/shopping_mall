package me.ahn.management.util;
...
public class TokenUtil {
     ...
     * @return
     */
    public static String createToken(TB_USER_INFO tbUserInfo) {
        // 1. 设置Token过期时间
        ...
        log.info("正在创建Token的用户>>>>>>>>>>" + tbUserInfo.toString());
        ...
        String token = JWT.create() //创建Token实例
                ...
                .withAudience(tbUserInfo.getUSER_SEQ().toString())
                .withExpiresAt(date) //过期时间
                .sign(algorithm); //签名

        log.info(" 用户[" + tbUserInfo.getUSER_SEQ() + "] 创建Token>>>>>>>>>>" + token);
        return token;
    }
